...
#provide data frame first, preferred way (can call multiple variables)
irradiance.regression <- lm(
  data = PeterPaul.chem.nutrients,
  irradianceWater ~ depth)
summary(irradiance.regression) #ask for summary
...
>  slope of coefficient line = -95.809 (negative relationship -> increase depth, decrease light)
p-value < 0.05, our coefficients are statistically different than zero, it's worthwhile trying to 
estimate the light level of lakes based on depth. 
...
irradiancebydepth2 <- 
  ...
  scale_y_log10() +    #add log scale on y 
  geom_point() 
...
``` {r, fig.height = 3, fig.width = 4}
#same lm function, call data and specific subset (lake name), formula, vars 
...
#just include columns of interest, omit NAs
PeterPaulnutrients <- 
  PeterPaul.chem.nutrients %>%
  select(tn_ug:po4) %>%
  na.omit()
PeterPaulCorr <- cor(PeterPaulnutrients)
#thinner scatter plot, shape = represent the scatter plot of one against another, thinner = most linear. color strength and sign = -1 (red) to 1 (blue)
corrplot(PeterPaulCorr, method = "ellipse")

#mirror matrix, so show the number and shape 
...
#for removing each variable, what is the AIC (looking at if it's a better regression than removing none)
#go through removing vars w/ worst AIC 
