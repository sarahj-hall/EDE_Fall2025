...
NTL.phs.data.1999 <- filter(NTL.phys.data, year4 %in% c(1999))

# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
NTL.phs.data.Tuesday.Lake.1990.1999 <- 
...
> Answer: Rows number are not static, and we don't set them
...
surface_water_temp <- NTL_ex3 %>% 
  filter(depth == 0) %>% 
  summarize(mean = mean(surface_water_temp) )
