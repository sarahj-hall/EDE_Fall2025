...
getwd()
here() #double check that they are both the same, checking that project location is correct (should be EDEFall2025)
...
USGS.flow.data$datetime <- as.Date(USGS.flow.data$datetime, format = '%Y-%n-%d')
class(USGS.flow.data$datetime)
...
```{r, fig.height = 3, fig.width = 4}
# histogram of the mean discharge
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean))

# changes bin width to 10
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), binwidth = 10)

# changes number of bins to 10, x-axis units 
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), bins = 20)

# new graph, x is discharge mean, bin width is 10, and define extent of the x-axis 
...
```{r, fig.height = 3, fig.width = 4}
#new plot, with 3 lines, setting color & bins in each geom  
ggplot(USGS.flow.data) +
...
#creates new plot, with legend on the left 
ggplot(USGS.flow.data) +
