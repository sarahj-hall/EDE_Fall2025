...
```{r}
#could also call the column number [8]
#first date of observation, frequency of observation (365, daily)
...
```{r}
?stl
#univariate time series, even if you have more then 1 var, need to run decomposition for each variable (can't done more than 1 variables at a time)
#s.window: define (s.window = periodic, specified when ts object is created), or s.window = 365 (days) s.window = 12 (months)
...
#Object time series (which has seasonal, trend, and remainder)
#create a DF with the three columns
EnoDischarge.late_Components <- as.data.frame(EnoDischarge.late_Decomposed$time.series[,1:3])

#adding two new columns, observed discharge and date
...
#can see that the seasonal component goes below the 0 line 
ggplot(EnoDischarge.late_Components) +
...
#na.approx used to replace missing data by approximation 
#packagename::nameoffunction (don't need to library package)
wind_data_clean <- 
  wind_data %>% 
...
wind_data_ts <- ts(wind_data_clean$AWND.clean,
                   start=c(f_year,f_month), #using defined values from above
                   frequency=12) 
...
# Run SMK test
wind_data_trend1 <- Kendall::SeasonalMancnKendall(wind_data_ts)

# Inspect results
#p-value is < 0.05, so reject H0 data is stationary, there is a trend
wind_data_trend1
summary(wind_data_trend1)

wind_data_trend2 <- trend::smk.test(wind_data_ts)
# Inspect results
#look at season changes individually, s (change) and p-value 
wind_data_trend2
summary(wind_data_trend2)

#Visualization
#plot wind data, add linear regression 
#there is a negative slope, showing decrease in winds speeds over time
wind_data_plot <-
