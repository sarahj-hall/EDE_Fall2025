...
#when you load compounded/umbrella packages it loads multiple libraries 
...
> Answer: Errors come up showing packages with the same names (ex: dplyr has a filter, and this package called stats also has a filter) to use the other one, do stats::filter 
...
> ANSWER: The README file is found in the metadata folder, this information is very important for how the data was produced and allows for another used to replicate the dataset for the same year or for other years. 
...
colnames(USGS.flow.data) #column names, could also use names()
...
USGS.flow.data[30000:30005, c(3, 8, 14)] #look at specific sample of rows, and columns 

class(USGS.flow.data$datetime) #$ pulls out a specific column and pulls out a value
...
> Answer: they are automatically given NA for numerical data
...
```{r Finding NAs}
summary(USGS.flow.data$discharge.mean) #use$ to get summary of a specific column 
...
```{r Removing NAs}
USGS.flow.data.complete <- na.omit(USGS.flow.data) #removes all rows with missing data is removed 
dim(USGS.flow.data)
dim(USGS.flow.data.complete)

mean(USGS.flow.data$discharge.mean) #can't calculate mean with NA values 
mean(USGS.flow.data.complete$discharge.mean) #calculate mean w/ new DF with no NA values 
...
Alternatively, we can tell R to import it as a factor when we read the data in (usually better)

```{r}
USGS.flow.data2  <- read.csv(
  file = here("./Data/Raw/USGS_Site02085000_Flow_Raw.csv"),
  stringsAsFactors = TRUE) 
  colClasses = c('site_number'='factor')
```
...
format(today, format = "%a,%m,%d")
#format(today, format = "")

USGS.flow.data$datetime <- as.Date(USGS.flow.data$datetime, format = "%m/%d/%y") #takes data and puts into date column in data, specifies month/day/year 
```
...
```{r Fixing dates}
# 
USGS.flow.data$datetime <- format(USGS.flow.data$datetime, "%y%m%d") #changes format to year,month,day
...
write.csv(
  USGS.flow.data, 
  file = here("./Data/Processed/USGS_Site02085000_Flow_Processed.csv"), #. evaluates to here, starts in wd 
  row.names=FALSE)
