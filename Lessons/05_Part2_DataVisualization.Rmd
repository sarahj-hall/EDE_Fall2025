...
```{r, fig.height = 3, fig.width = 4, warning = FALSE}
#base size is 11 in default ggplot, increase size to 14 
#set default theme, stores values 
...
#or define theme first, then define as usual in later plots 
theme_set(mytheme)
...
```{r, fig.height = 3, fig.width = 4, warning = FALSE}
#add a horizontal line that cross y int at 50, lty = define type of the line
#add a horizontal line that cross y int at 100
#alpha: transparency and size of points 
#add text to plot, define x point and y point 
#change scale of x-axis (give range)
#
...
NvsP2 <-
  ggplot(PeterPaul.chem.nutrients, aes(x = tp_ug, y = tn_ug, color = depth, shape = lakename)) +
  geom_point(alpha = 0.7, size = 2.5) + #size and transparency of the points
  #labs() + # change your legend labels here
  labs(color = "Depth", shape = "Lake Name") + #two legends, one associated w/ color and shape
  scale_shape_manual(values = c(15, 17)) +
  #scale_color_distiller(palette = "Blues", direction = 1) + # use scale_color_brewer for discrete variables
  scale_color_viridis(option = "magma", direction = -1) + #change color using viridis color 
...
library(cowplot)
#rel_heights = c(first plot height, 2nd plot height), different sizes for the plots
